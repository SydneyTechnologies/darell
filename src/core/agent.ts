...
function buildPrompt(root: string): string {
  return [
    ...
    `You can only act within the workspace root: ${root}.`,
    'Return a JSON object with a summary and an ordered list of actions.',
...
export async function runAgent(options: RunAgentOptions): Promise<void> {
  ...
  const response = await client.chat.completions.create({
    model,
    messages: [
      { role: 'system', content: buildPrompt(options.root) },
  ...
  const ctx = { root: options.root };
  for (const action of plan.actions) {
    const description = `${action.type}: ${'path' in action ? action.path : ''} ${'from' in action ? action.from : ''} ${'to' in action ? action.to : ''} ${'command' in action ? action.command : ''} ${'args' in action ? action.args.join(' ') : ''}`.trim();
    emit(options, { type: 'action', message: description });

    if (!options.autoApprove) {
      const confirm = options.confirm ?? ((next: AgentAction) => confirmAction(`Run ${next.type}? ${next.reason ? next.reason : ''}`.trim()));
    ...
    try {
      ...
      switch (action.type) {
        ...
        case 'append_file':
          ...
          break;
        case 'create_file':
          result = await createFile(ctx, action.path, action.content, action.overwrite ?? false);
          break;
        case 'delete_file':
          result = await deleteFile(ctx, action.path);
          break;
        case 'replace_in_file':
          result = await replaceInFile(ctx, action.path, action.find, action.replace, action.replaceAll ?? true);
          break;
        case 'list_dir':
          result = await listDir(ctx, action.path, action.recursive ?? false, action.includeHidden ?? false);
          break;
        case 'file_info':
      ...
      emit(options, { type: 'error', message: String(error) });
    }
  }
}
